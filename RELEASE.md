...
CUMS (Classroom Upload Management System) v2.3.3 是一个专为机房环境设计的文件上传系统，采用**以科目为中心**的架构设计，支持多班级、多科目、多作业的上传管理。

**v2.3.3 是版本升级更新，继承了 v2.0.0 的所有功能。
...
```bash
# Windows
cums_2.3.3_20260128.exe

# Linux
chmod +x cums_2.3.3_20260128
./cums_2.3.3_20260128
...
|--------|------|--------|------|
| `version` | 版本号 | `"2.3.3"` | 是 |
| `server_addr` | 服务器地址 | `":3000"` | 是 |
| `admin_enabled` | 是否启用管理后台 | `false` | 否 |
...
4. **迁移配置**
   - 从 `cums/config.json` 复制到根目录
   - 更新版本号为 `"2.0.0"`
   - 如需启用管理后台，添加 `admin_enabled` 和 `admin_password`

5. **迁移数据**（可选）
   - 从 `cums/uploads/` 复制到根目录 `uploads/`
   - 从 `cums/logs/` 复制到根目录 `logs/`
...
| 平台 | 文件名 | 大小 |
|------|--------|------|
| Windows | cums_2.3.3_20260128.exe | ~6.5 MB |
| Linux | cums_2.3.3_20260128 | ~6.2 MB |
| macOS | cums_2.3.3_20260128 | ~6.3 MB |
